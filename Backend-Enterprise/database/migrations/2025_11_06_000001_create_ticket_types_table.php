...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('ticket_types', function (Blueprint $table) {
            $table->id();
            $table->string('name'); // Nombre del tipo (Mantenimiento Preventivo, Falla de Inversor, etc.)
            $table->string('description')->nullable(); // Descripción del tipo
            $table->string('color')->default('#3b82f6'); // Color para mostrar en interfaz
            $table->enum('priority_level', ['baja', 'media', 'alta', 'critica'])->default('media'); // Prioridad por defecto para este tipo
            $table->timestamps();
