...
const DetalleCotizacion = () => {
    ...
    const fetchCotizacion = async () => {
      try {
        ...
        if (response.success) {
          ...
          console.log('🔄 Cotización transformada:', transformedCotizacion);
  ...
  const transformCotizacionData = (apiData) => {
    console.log('📥 Datos recibidos del backend:', JSON.stringify(apiData, null, 2));
    
    // Verificar que los datos requeridos estén presentes
    if (!apiData) {
      console.error('❌ Datos de cotización inválidos o vacíos');
      return null;
    }
    ...
    const transformedData = {
      ...
      cliente: {
        ...
        address: `${apiData.client?.address || 'Dirección no especificada'}`.trim(),
      ...
      suministros: Array.isArray(apiData.products) ? apiData.products.map(product => {
        ...
        const descripcionFinal = `${product.brand || ''} ${product.model || ''}`.trim() || getProductName(product.product_type, product.product_id) || 'Producto sin descripción';
  ...
  const getSuministrosByType = (tipo) => {
    switch (tipo) {
      case 'Panel Solar': return suministrosDisponibles.paneles;
      case 'Inversor': return suministrosDisponibles.inversores;
      case 'Batería': return suministrosDisponibles.baterias;
      default: return [];
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-600">Detalles de la cotización</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="font-semibold text-slate-900">{cotizacion?.cliente?.name || 'Nombre no disponible'}</p>
              <p className="text-sm text-slate-600">{cotizacion?.cliente?.type === 'empresa' ? 'Empresa' : (cotizacion?.cliente?.type === 'residencial' ? 'Residencial' : 'Tipo desconocido')}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="font-medium text-slate-900">{cotizacion?.cliente?.email || 'No especificado'}</p>
            </div>
            
            <div>
              <p className="text-sm text-slate-600">Teléfono</p>
              <p className="font-medium text-slate-900">{cotizacion?.cliente?.phone || 'No especificado'}</p>
            </div>
            
            <div>
              <p className="text-sm text-slate-600">Dirección</p>
              <p className="font-medium text-slate-900">{cotizacion?.cliente?.address || 'No especificada'}</p>
            </div>
