  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!clientIdFromProject) {
        alert('El proyecto seleccionado no tiene un cliente asociado válido. Por favor, selecciona otro proyecto.');
        return;
      }

      const ticketData = {
        project_id: formData.proyectoId,
        client_id: clientIdFromProject,
        title: formData.titulo,
        description: formData.descripcion,
        type_id: formData.tipo || 1, // Valor predeterminado si no se selecciona
        priority_id: formData.prioridad || 1, // Valor predeterminado si no se selecciona
        status_id: formData.tipo ? (formData.tipo == '3' ? 2 : 1) : 1, // Valor predeterminado o basado en tipo
        assigned_to: formData.tecnicoAsignado.length > 0 ? formData.tecnicoAsignado[0] : null, // Asignar al primer técnico si hay alguno seleccionado
