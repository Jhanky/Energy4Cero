...
const VistaServicioTecnico = () => {
  ...
  const ticketsPorTipo = tiposTicket.map(tipo => ({
    nombre: tipo.nombre,
    cantidad: tickets.filter(t => t.type_id === tipo.id).length,
    color: tipo.color
  })).filter(d => d.cantidad > 0);

  const ticketsPorEstado = estadosTicket.map(estado => ({
    nombre: estado.nombre,
    cantidad: tickets.filter(t => t.status_id === estado.id).length,
    color: estado.color
  })).filter(d => d.cantidad > 0);
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${obtenerColorPrioridad(ticket.ticket_priority?.id || ticket.priority_id)} text-white`}>
